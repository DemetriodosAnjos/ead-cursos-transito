...
class Database {
  ...
  private mapSupabaseRowToCourse(row: any): Course {
    const rawPrice = row.price !== undefined && row.price !== null ? Number(row.price) : 169;
    const costPrice = row.cost_price !== undefined && row.cost_price !== null ? Number(row.cost_price) : undefined;
    const profitPercent = row.profit_percent !== undefined && row.profit_percent !== null ? Number(row.profit_percent) : undefined;
    ...
    return {
      ...
      categoryLabel: row.category_label || row.categoryLabel || 'Formação Especializada',
      ...
      workloadHours: row.workload_hours !== undefined && row.workload_hours !== null ? Number(row.workload_hours) : 50,
      detranApproval: row.detran_approval || row.detranApproval || 'Homologado Resolução CONTRAN e DETRAN PR',
  ...
  private async initSupabaseCourses() {
    ...
    if (!supabase) {
      console.warn('[SUPABASE] Não conectado. Usando catálogo estático como contingência.');
    ...
    try {
      ...
      if (!error && data && data.length > 0) {
        ...
        console.log(`[SUPABASE] Catálogo de ${this.courses.length} cursos sincronizado exclusivamente do Supabase PostgreSQL.`);
      } else if (!error && (!data || data.length === 0)) {
        console.log('[SUPABASE] Tabela courses vazia. Semeando catálogo inicial...');
  ...
  async updateCourse(id: string, updates: Partial<Course>): Promise<Course | null> {
    ...
    if (supabase) {
      try {
        ...
        if (updates.categoryLabel !== undefined) payload.category_label = updated.categoryLabel;
        if (updates.description !== undefined) payload.description = updated.description;
        if (updates.fullDescription !== undefined) payload.full_description = updated.fullDescription;
        if (updates.duration !== undefined) payload.duration = updated.duration;
        if (updates.workloadHours !== undefined) payload.workload_hours = updated.workloadHours;
        if (updates.detranApproval !== undefined) payload.detran_approval = updated.detranApproval;
        if (updates.modality !== undefined) payload.modality = updated.modality;
        if (updates.thumbnail !== undefined) payload.thumbnail = updated.thumbnail;
        if (updates.backdrop !== undefined) payload.backdrop = updated.backdrop;
        if (updates.badge !== undefined) payload.badge = updated.badge;
        if (updates.requirements !== undefined) payload.requirements = updated.requirements;
        if (updates.modules !== undefined) payload.modules = updated.modules;
        if (updates.isFeatured !== undefined) payload.is_featured = updated.isFeatured;
        ...
        if (error) {
          ...
          console.log(`[SUPABASE] Curso [${id}] atualizado com sucesso no banco! Novo preço: R$ ${updated.price}`);
        }
      } catch (err) {
  ...
  async toggleCourseActive(id: string): Promise<{ success: boolean; isActive: boolean; course?: Course }> {
    ...
    if (supabase) {
      try {
        await supabase
          .from('courses')
          .update({ is_active: course.isActive, updated_at: new Date().toISOString() })
          .eq('id', id);
